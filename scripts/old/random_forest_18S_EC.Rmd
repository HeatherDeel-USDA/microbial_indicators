...
SHAI18S_ml_SOM <- SHAI18S_ml[,c(79,154:1282)]

# filter NAs
SHAI18S_ml_SOM <- SHAI18S_ml_SOM %>%
  filter(!is.na(organic_matter_rating))

# read in tuning results (generated in random_forest_18S_EC_tuning.Rmd on Scinet and transferred to local one drive)
# change the set_args parameters before running the 25 loops
rf_tune_results <- readRDS("machine_learning/18S_EC/SOM_model_results/SHAI18S_ml_SOM_tune_results.RDS")
param_final <- rf_tune_results %>%
  select_best(metric = "mae")
param_final
...
for (i in 1:25) {
  soil_split <- initial_split(SHAI18S_ml_SOM, prop = 4/5)
  soil_split
  # 252 samples in training, 64 in testing, 316 total
  ...
  soil_recipe <- recipe(organic_matter_rating ~ ., data = SHAI18S_ml_SOM)
  soil_recipe
  # 1129 predictors
  ...
  # save the fit
  saveRDS(rf_fit, paste("machine_learning/18S_EC/SOM_model_results/ml_all_EC_fit", i, ".RDS", sep = ""))
  ...
  # graph a regression of predicted vs observed organic_matter_rating values
  ...
  # save R^2 and p-values to files
  write.table(summary(SOM_EC_lm)$adj.r.squared, file = "machine_learning/18S_EC/SOM_model_results/SOM_r2_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
  
  write.table(summary(SOM_EC_lm)$coef[2,4], file = "machine_learning/18S_EC/SOM_model_results/SOM_p_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
  
  # save plot
  ggsave(paste("machine_learning/18S_EC/SOM_model_results/ml_all_EC_pred_vs_obs", i, ".tiff", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "tiff")
  
  # fitting the final model
  # uses all data that can be tested on a new data set
  final_model <- fit(rf_workflow, SHAI18S_ml_SOM)
  
  # save the final model
  saveRDS(final_model, paste("machine_learning/18S_EC/SOM_model_results/ml_all_EC_final_model", i, ".RDS", sep = ""))
  
  # variable importance
  ranger_obj <- pull_workflow_fit(final_model)$fit
  ranger_obj
  var_importance <- as.data.frame(ranger_obj$variable.importance)
  write.csv(var_importance, paste("machine_learning/18S_EC/SOM_model_results/var_importance_EC", i, ".csv", sep = ""), row.names = TRUE)
}
```
...
SHAI18S_ml_AGGSTAB <- SHAI18S_ml[,c(77,154:1282)]

# filter NAs
SHAI18S_ml_AGGSTAB <- SHAI18S_ml_AGGSTAB %>%
  filter(!is.na(aggregate_stability_rating))

# read in tuning results (generated in random_forest_18S_EC_tuning.Rmd on Scinet and transferred to local one drive)
# change the set_args parameters before running the 25 loops
rf_tune_results <- readRDS("machine_learning/18S_EC/AGGSTAB_model_results/SHAI18S_ml_AGGSTAB_tune_results.RDS")
param_final <- rf_tune_results %>%
  select_best(metric = "mae")
param_final
...
for (i in 1:25) {
  soil_split <- initial_split(SHAI18S_ml_AGGSTAB, prop = 4/5)
  soil_split
  # 252 samples in training, 64 in testing, 316 total
  ...
  soil_recipe <- recipe(aggregate_stability_rating ~ ., data = SHAI18S_ml_AGGSTAB)
  soil_recipe
  # 1129 predictors
  
  # specify the model
  rf_model <- rand_forest() %>% 
    set_args(mtry = 282, trees = 500, min_n = 3) %>%
    set_engine("ranger", importance = "impurity") %>%
    set_mode("regression")
  ...
  # save the fit
  saveRDS(rf_fit, paste("machine_learning/18S_EC/AGGSTAB_model_results/ml_all_EC_fit", i, ".RDS", sep = ""))
  ...
  # graph a regression of predicted vs observed aggregate_stability_rating values
  ...
  # save R^2 and p-values to files
  write.table(summary(AGGSTAB_EC_lm)$adj.r.squared, file = "machine_learning/18S_EC/AGGSTAB_model_results/AGGSTAB_r2_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
  
  write.table(summary(AGGSTAB_EC_lm)$coef[2,4], file = "machine_learning/18S_EC/AGGSTAB_model_results/AGGSTAB_p_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
  
  # save plot
  ggsave(paste("machine_learning/18S_EC/AGGSTAB_model_results/ml_all_EC_pred_vs_obs", i, ".tiff", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "tiff")
  
  # fitting the final model
  # uses all data that can be tested on a new data set
  final_model <- fit(rf_workflow, SHAI18S_ml_AGGSTAB)
  
  # save the final model
  saveRDS(final_model, paste("machine_learning/18S_EC/AGGSTAB_model_results/ml_all_EC_final_model", i, ".RDS", sep = ""))
  
  # variable importance
  ranger_obj <- pull_workflow_fit(final_model)$fit
  ranger_obj
  var_importance <- as.data.frame(ranger_obj$variable.importance)
  write.csv(var_importance, paste("machine_learning/18S_EC/AGGSTAB_model_results/var_importance_EC", i, ".csv", sep = ""), row.names = TRUE)
}
```
