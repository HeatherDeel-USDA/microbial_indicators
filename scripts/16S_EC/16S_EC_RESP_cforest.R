...
ml_EC_16S_RESP <- ml_EC_16S[,c(2:2445,2515,2547,2524)]
...
names(ml_EC_16S_RESP) <- gsub(":","_", names(ml_EC_16S_RESP))
names(ml_EC_16S_RESP) <- gsub("\\.","_", names(ml_EC_16S_RESP))

# split into train and test (4/5 proportion)
ml_EC_16S_RESP$id <- 1:nrow(ml_EC_16S_RESP)
train <- ml_EC_16S_RESP %>% dplyr::sample_frac(0.80)
test <- dplyr::anti_join(ml_EC_16S_RESP, train, by = 'id')
...
saveRDS(cf.resp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/RESP_model_results_clay_climate/cf.resp", args[1], ".RDS", sep = ""))
...
test.resp <- data.frame(test[,2447])
colnames(test.resp)[1] <- "resp.obs"
test.resp <- rownames_to_column(test.resp, var = "id")
cf.pvso <- merge(cf.pred, test.resp, by = "id")

saveRDS(cf.pvso, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/RESP_model_results_clay_climate/cf_obs_vs_pred", args[1], ".RDS", sep = ""))
...
ggsave(paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/RESP_model_results_clay_climate/RESP_EC_pred_vs_obs", args[1], ".pdf", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "pdf")
...
write.table(cbind(task_num,r2_val,p_val), 
            file = "/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/RESP_model_results_clay_climate/resp.ec.stats.csv", 
...
write.csv(resp.imp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/RESP_model_results_clay_climate/RESP_EC_var_importance", args[1], ".csv", sep = ""), row.names = TRUE)
