...
#            'ph', 'p', 'k', 'mg', 'fe', 'mn', 'zn', 'clay', 'DNA')
myDatas <- c('EC_RA_corr', 'GIBBs_RA_corr')
...
modules <- read.csv('res_node_table_0.9.csv')

myVars <- c('ace', 'SOM', 'activeC', 'resp', 'agg_stab', 'water_cap', 'ph', 'p', 
            'k', 'mg', 'fe', 'mn', 'zn')
myData <- 'EC_RA_corr'
...
for (myVar in myVars) {
  ...
  gDF <- gDF %>%
    left_join(modules[,c('X', 'module')], by=c("Predictor"="X")) %>%
    mutate(Predictor=fct_reorder(Predictor, VIP, .fun=median, .desc=TRUE)) # sort levels by median VIP
  ...
  maxs <- aggregate(VIP ~ Predictor, gDF, max)
  means$y <- maxs$VIP + 0.025
  
  means2 <- aggregate(VIP ~ Predictor + module, gDF, median)
  means2$Category <- 'Other'
  ...
  p <- ggplot(gDF, aes(x=Predictor, y=VIP, fill=Category)) +
    ...
    geom_text(data = means, aes(label = VIP, y = y), size=2.8, fontface='italic', angle=30) +
    geom_text(data = means2, aes(label = module, y = -0.025), size=2.8, angle=0) +
    theme_bw() +
...
P.filt <- filter_taxa(P, function(x) mean(x) > 0.01, TRUE) # 1506 EC's
  
EC.meco <- phyloseq2meco(P.filt)

t1 <- clone(EC.meco)

### start here - want the cooccurrence network to only include coocurrences of the same upward or downward trend
# spearman is supposed to only calculate correlations between monotonic relationships, so I think this is okay, but trying to verify
# e.g., do all enzymes in module 1 have a monotonic relationship to each other?
t1 <- trans_network$new(dataset=EC.meco, cor_method="spearman", 
                        taxa_level='order', filter_thres = 0)
t1$cal_network(COR_p_thres = 0.01, COR_cut=0.8, add_taxa_name='order', 
...
#write.csv(t1$res_node_table, 'res_node_table_0.8.csv')

colors <- rep(paletteer::paletteer_d("ggsci::default_igv", 51),6)
...
use_network_layout$kingdom <- gsub('k__', '', use_network_layout$kingdom)
g <- ggraph(use_network_layout) +
  geom_edge_link(aes(col = label, width = weight), alpha = 0.8) +
  geom_node_point(aes(fill = kingdom, size = degree), shape = 21) +
  scale_fill_manual(values = colors, name = 'Class') +
  scale_edge_width(range = c(0.5, 2)) +
  theme_void() #+
  #theme(legend.position = 'none')
ggsave(paste0("../../dan_out_2/", "All_", myData, "_network_0.8.png"), device='png', dpi=600, height=8, width=11)


# meta <- ml_EC_16S[,c('SOM','ace','resp','activeC','ph', 'p', 'k','mg','fe','mn','zn','clay')]
# row.names(meta) <- ml_EC_16S$SampleID
# t2 <- trans_env$new(dataset = EC.meco, add_data = meta)
# t2$cal_cor(add_abund_table = t1$res_eigen)
# p <- t2$plot_cor()

#random forest with modules
meta <- ml_EC_16S[,c('SOM','ace','resp','activeC','ph', 'p', 'k','mg','fe','mn','zn','clay', 'DNA', 'ClimateZ')]
row.names(meta) <- ml_EC_16S$SampleID

df <- cbind(meta, t1$res_eigen)
df$ClimateZ <- factor(df$ClimateZ)

mod <- lm(M1 ~ ClimateZ * clay, data=df)
anova(mod)

ggplot(df, aes(x=clay, y=M1, fill=ClimateZ, color=ClimateZ)) + 
  geom_point(shape=21) +
  stat_smooth(method = "lm", se=FALSE)

ggplot(df, aes(x=ClimateZ, y=M1, fill=ClimateZ)) + 
  geom_boxplot()
