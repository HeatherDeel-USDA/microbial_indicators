...
counter <- 1
for (myVar in myVars) {
  df <- read.csv(paste0("../../dan_out/", myVar, "_", myData, "_varimp.csv"), header=FALSE)
  names(df) <- c('Predictor', 'VIP', 'Run')
  
  gDF <- df %>%
    left_join(gibbs, by=c("Predictor"="EC")) %>% 
    mutate(Predictor=fct_reorder(Predictor, VIP, .fun=mean, .desc=TRUE)) # sort levels by mean VIP
  
  gDF <- gDF %>% group_by(Predictor) %>%
    mutate(Var = mean(VIP)) %>%
    filter(Var > -1) %>% select(-Var) # filter out poor indicators
...
#df <- allDF %>% 
#  mutate(flag = TRUE) %>% 
#  complete(myVar, Predictor, fill = list(flag = FALSE))

#ggplot(df, aes(myVar, Predictor, fill = VIP_avg)) + 
#  geom_tile()
```
...
t1 <- trans_network$new(dataset=EC.meco, cor_method="spearman", filter_thres = 0)
t1$cal_network(COR_p_thres = 0.01, COR_cut=0.7) # 0.69
...
g <- ggraph(use_network_layout) +
  ...
  geom_node_point(aes(fill = module.x, linewidth = degree), shape = 21) +
