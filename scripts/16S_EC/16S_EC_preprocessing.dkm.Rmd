...
ml_EC_16S <- readRDS('../../machine_learning/EC_Total_corr/ml_EC_Total_corr.RDS')
ml_EC_16S <- ml_EC_16S %>%
  filter(str_detect(SampleID, "^NRCS"))

# lasso regression
y <- ml_EC_16S$ace
x <- data.matrix(ml_EC_16S[,grep("EC:", colnames(ml_EC_16S)),])
cv_model <- cv.glmnet(x, y, alpha = 1)
best_lambda <- cv_model$lambda.min
best_lambda
plot(cv_model) 
best_model <- glmnet(x, y, alpha = 1, lambda = best_lambda)
coeffs <- as.matrix(coef(best_model))
keep_X <- as.vector(rownames(coeffs)[coeffs!=0])[-1]

# get selected predictors
ml_EC_16S_ACE <- ml_EC_16S %>%
  select(keep_X, ace)
...
for (myRun in seq(1,25,1)) {
  ...
  my_cforest_control <- cforest_control(teststat = "quad",
      testtype = "Univ", mincriterion = 0, ntree = 500, mtry = 3,
      replace = FALSE)
  ...
  #ggsave(paste("../../dan_out/ACE_EC_RA_obs_vs_pred", myRun, ".png", sep = ""), 
  #       unit = "in", width = 6, height = 6, dpi = 300, device = "png")
  ...
  write.table(cbind(myRun, r2_val, p_val), 
              file = "../../dan_out/ACE_EC_Total_corr_stats.csv", 
              col.names = c("task_number","r2_val","p_val"),
              append = TRUE, sep = ",", row.names = FALSE)
  
  # variable importances
  ace.imp <- varimp(cf.ace)
  ace.imp <- as.data.frame(ace.imp)
  names(ace.imp) <- 'vimp'
  ace.imp <- ace.imp %>% arrange(desc(vimp))
  ace.imp <- tibble::rownames_to_column(ace.imp, "EC")
  ace.imp$Run <- myRun
  
  write.table(ace.imp, 
              file = "../../dan_out/ACE_EC_Total_corr_varimp.csv",
              append = TRUE, sep = ",", row.names = FALSE)
  
  #pd <- edarf::partial_dependence(cf.ace, ace.imp$EC[1:3], interaction=FALSE)
  #p <- edarf::plot_pd(pd)
  #ggsave(paste("../../dan_out/ACE_EC_RA_corr_pd", myRun, ".png", sep = ""), 
  #       unit = "in", width = 8, height = 8, dpi = 300, device = "png")
  print(paste0("Finished run: ", myRun))
}
