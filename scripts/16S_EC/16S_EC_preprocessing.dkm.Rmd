...
#myVars <- c('ace', 'SOM', 'activeC', 'resp', 'agg_stab', 'water_cap', 'SH_rating', 'Overall', 'ph', 'p', 'k', 'mg', 'fe', 'mn', 'zn')
myVars <- c('SH_rating', 'ph', 'p', 'k', 'mg', 'fe', 'mn', 'zn')

myDatas <- c('EC_RA', 'EC_RA_corr', 'EC_Total_corr', 'TAX_RA')
#myData <- 'TAX_RA'
  ...
  for (myData in myDatas) {
    ...
    if (myData == "TAX_RA") {
      ...
      data <- ml_EC_16S %>%
        ...
        select(grep("EC:", colnames(ml_EC_16S)), myVar)
    }
    ...
    for (myRun in seq(1,25,1)) {
      ...
      p1 <- ggplot(cf.pvso, aes(x = obs, y = pred)) +
        geom_point() +
        stat_smooth(method = "lm", se = TRUE, level = 0.95) +
        labs(title = paste("Adj R2 =",signif(summary(res.lm)$adj.r.squared, 2),
                           " P =",signif(summary(res.lm)$coef[2,4], 2)),
      ...
      # #p <- edarf::plot_pd(pd)
      # ggsave(paste("../../dan_out/", myVar, "_", myData, "_pd", myRun, ".png", sep = ""), 
