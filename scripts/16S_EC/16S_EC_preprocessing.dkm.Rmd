...
myDatas <- c('GIBBs_RA_corr', 'GIBBs_Total_corr')
...
gDF <- df %>%
  ...
  mutate(Short=fct_reorder(Short, VIP, .fun=mean, .desc=TRUE))

gDF <- gDF %>% group_by(Predictor) %>%
  mutate(Var = mean(VIP)) %>%
  filter(Var > 0.01) %>% select(-Var)

colors <- paletteer_d(`"ggsci::category20_d3"`)
p <- ggplot(gDF, aes(x=Short, y=VIP, fill=Category)) +
  scale_fill_manual(values=colors, na.value="white") +
  geom_boxplot() + 
  theme_bw() +
  theme(axis.text.x=element_text(angle=90)) +
  labs(x="", y="Variabile Importance")
p
