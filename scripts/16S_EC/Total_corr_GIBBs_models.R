...
for (myVar in myVars) {
  ...
  # GIBBs enzymes 
  gibbs <- read.csv("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/GIBBs.EC.numbers.csv")
  gibbs <- gibbs %>%
    filter(Missing != 'x')
  ...
  write.table(cbind(args[1], r2_val, p_val, myVar, more_stats),
              file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/", myVar, "_model_results_total_corr_GIBBs/", myVar, "_EC_total_corr_GIBBs_stats", ".csv", sep = ""),
  ...
  write.table(imp,
              file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/", myVar, "_model_results_total_corr_GIBBs/", myVar, "_total_corr_GIBBs_varimp", args[1], ".csv", sep = ""),
  ...
  for (EC in seq(1,min(10,length(imp$EC)),1)) {
    ...
    write.table(pd, 
                file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/", myVar, "_model_results_total_corr_GIBBs/", myVar, "_total_corr_GIBBs_pd", "_Predictor_", imp$EC[EC], ".csv", sep = ""),
                col.names=FALSE,
