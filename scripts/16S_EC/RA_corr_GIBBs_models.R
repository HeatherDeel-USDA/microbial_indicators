...
for (myVar in myVars) {
  ...
  for (EC in seq(1,min(10,length(imp$EC)),1)) {
    ...
    write.table(pd, 
                file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/", myVar, "_model_results_RA_corr_GIBBs/", myVar, "_RA_corr_GIBBs_pd", "_Predictor_", imp$EC[EC], ".csv", sep = ""),
                col.names=FALSE,
