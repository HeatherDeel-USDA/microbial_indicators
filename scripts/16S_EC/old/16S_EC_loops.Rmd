...
######### need to do a lot of editing this script, still has .corr vals, and will need to change model parameters after retuning, as well as making graphs for models with clay+climate
#### and was messing with ace
...
### Predict SEMWISE rating, including clay and climate
# Run 25 times, with outputs 1-25, to account for lucky/unlucky splits after tuning
```{r}
ml_EC_16S <- readRDS("machine_learning/16S_EC/ml_EC_16S.RDS")
...
# read in tuning results (generated in individual scripts in scripts/16S_EC, run on Scinet, and transferred to local one drive)
# change the set_args parameters before running the 25 loops
rf_tune_results <- readRDS("machine_learning/16S_EC/SEMWISE_model_results_clay_climate/ml_EC_16S_SEMWISE_tune_results_clay_climate.RDS")
param_final <- rf_tune_results %>%
  select_best(metric = "mae")
param_final
...
for (i in 1:25) {
  ...
  # specify the model
  rf_model <- rand_forest() %>% 
    set_args(mtry = 613, trees = 250, min_n = 3) %>%
    set_engine("ranger", importance = "impurity") %>%
    set_mode("regression")
  ...
  # save the fit
  saveRDS(rf_fit, paste("machine_learning/16S_EC/SEMWISE_model_results_clay_climate/ml_all_EC_fit", i, ".RDS", sep = ""))
  ...
  # save R^2 and p-values to files
  write.table(summary(SEMWISE_EC_lm)$adj.r.squared, file = "machine_learning/16S_EC/SEMWISE_model_results_clay_climate/SEMWISE_r2_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
  
  write.table(summary(SEMWISE_EC_lm)$coef[2,4], file = "machine_learning/16S_EC/SEMWISE_model_results_clay_climate/SEMWISE_p_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
  
  # save plot
  ggsave(paste("machine_learning/16S_EC/SEMWISE_model_results_clay_climate/ml_all_EC_pred_vs_obs", i, ".tiff", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "tiff")
  ...
  # save the final model
  saveRDS(final_model, paste("machine_learning/16S_EC/SEMWISE_model_results_clay_climate/ml_all_EC_final_model", i, ".RDS", sep = ""))
  
  # variable importance
  ranger_obj <- pull_workflow_fit(final_model)$fit
  ranger_obj
  var_importance <- as.data.frame(ranger_obj$variable.importance)
  write.csv(var_importance, paste("machine_learning/16S_EC/SEMWISE_model_results_clay_climate/var_importance_EC", i, ".csv", sep = ""), row.names = TRUE)
}
```
...
# read in tuning results (generated in individual scripts in scripts/16S_EC, run on Scinet, and transferred to local one drive)
# change the set_args parameters before running the 25 loops
rf_tune_results <- readRDS("machine_learning/16S_EC/SOM_model_results_clay_climate/ml_EC_16S_SOM_tune_results_clay_climate.RDS")
param_final <- rf_tune_results %>%
  select_best(metric = "mae")
param_final
...
set.seed(14)
  ...
  for (i in 1:25) {
    ...
    # define a "recipe", i.e., the role of each variable in the model
    # predicting SOM, and all other variables (ECs) are predictors
    soil_recipe <- recipe(SOM ~ ., data = ml_EC_16S_SOM)
    soil_recipe
    
    # specify the model
    rf_model <- rand_forest() %>% 
      set_args(mtry = 1839, trees = 500, min_n = 5) %>%
      set_engine("ranger", importance = "impurity") %>%
      set_mode("regression")
    ...
    # save the fit
    saveRDS(rf_fit, paste("machine_learning/16S_EC/SOM_model_results_clay_climate/ml_all_EC_fit", i, ".RDS", sep = ""))
    ...
    # save R^2 and p-values to files
    write.table(summary(SOM_EC_lm)$adj.r.squared, file = "machine_learning/16S_EC/SOM_model_results_clay_climate/SOM_r2_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
    
    write.table(summary(SOM_EC_lm)$coef[2,4], file = "machine_learning/16S_EC/SOM_model_results_clay_climate/SOM_p_values.txt", append = TRUE, sep = "\t", row.names = FALSE, col.names = FALSE)
    
    # save plot
    ggsave(paste("machine_learning/16S_EC/SOM_model_results_clay_climate/ml_all_EC_pred_vs_obs", i, ".tiff", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "tiff")
    ...
    # save the final model
    saveRDS(final_model, paste("machine_learning/16S_EC/SOM_model_results_clay_climate/ml_all_EC_final_model", i, ".RDS", sep = ""))
    
    # variable importance
    ranger_obj <- pull_workflow_fit(final_model)$fit
    ranger_obj
    var_importance <- as.data.frame(ranger_obj$variable.importance)
    write.csv(var_importance, paste("machine_learning/16S_EC/SOM_model_results_clay_climate/var_importance_EC", i, ".csv", sep = ""), row.names = TRUE)
  }
```
