...
names(ml_EC_16S_WATERCAP) <- gsub(":","_", names(ml_EC_16S_WATERCAP))
names(ml_EC_16S_WATERCAP) <- gsub("\\.","_", names(ml_EC_16S_WATERCAP))

# split into train and test (4/5 proportion)
ml_EC_16S_WATERCAP$id <- 1:nrow(ml_EC_16S_WATERCAP)
train <- ml_EC_16S_WATERCAP %>% dplyr::sample_frac(0.80)
...
saveRDS(cf.water_cap, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/WATERCAP_model_results_clay_climate/cf.water_cap", args[1], ".RDS", sep = ""))
...
saveRDS(cf.pvso, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/WATERCAP_model_results_clay_climate/cf_obs_vs_pred", args[1], ".RDS", sep = ""))
...
ggsave(paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/WATERCAP_model_results_clay_climate/WATERCAP_EC_pred_vs_obs", args[1], ".pdf", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "pdf")
...
write.table(cbind(task_num,r2_val,p_val), 
            file = "/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/WATERCAP_model_results_clay_climate/water_cap.ec.stats.csv", 
...
write.csv(water_cap.imp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/WATERCAP_model_results_clay_climate/WATERCAP_EC_var_importance", args[1], ".csv", sep = ""), row.names = TRUE)
