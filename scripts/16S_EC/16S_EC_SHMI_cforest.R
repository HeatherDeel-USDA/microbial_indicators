...
ml_EC_16S_SHMI <- ml_EC_16S[,c(2:2445,2515,2547,2594)]
...
names(ml_EC_16S_SHMI) <- gsub(":","_", names(ml_EC_16S_SHMI))
names(ml_EC_16S_SHMI) <- gsub("\\.","_", names(ml_EC_16S_SHMI))

# split into train and test (4/5 proportion)
ml_EC_16S_SHMI$id <- 1:nrow(ml_EC_16S_SHMI)
train <- ml_EC_16S_SHMI %>% dplyr::sample_frac(0.80)
test <- dplyr::anti_join(ml_EC_16S_SHMI, train, by = 'id')
...
saveRDS(cf.SHMI2_rating, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/SHMI_model_results_clay_climate/cf.SHMI2_rating", args[1], ".RDS", sep = ""))
...
test.SHMI2_rating <- data.frame(test[,2447])
...
saveRDS(cf.pvso, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/SHMI_model_results_clay_climate/cf_obs_vs_pred", args[1], ".RDS", sep = ""))
...
ggsave(paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/SHMI_model_results_clay_climate/SHMI_EC_pred_vs_obs", args[1], ".pdf", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "pdf")
...
write.table(cbind(task_num,r2_val,p_val), 
            file = "/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/SHMI_model_results_clay_climate/SHMI2_rating.ec.stats.csv", 
...
write.csv(SHMI2_rating.imp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_EC/SHMI_model_results_clay_climate/SHMI_EC_var_importance", args[1], ".csv", sep = ""), row.names = TRUE)
