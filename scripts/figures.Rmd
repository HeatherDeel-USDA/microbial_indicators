...
##### this needs some work - struggling with doing this with varying EC names, have to figure out how to manually break up
### might need to try to import as phyloseq object?
```{r}
### subclass level data
...
sapply(unique(colnames(EC_subsubclass)), function(x) rowSums(EC_subsubclass[,grepl(x, colnames(EC_subsubclass))]))




# summarize subsubclass values - one way to try it that is currently not working because transpose keeping adding suffixes to duplicate column names
EC_subsubclass_t <- t(EC_subsubclass)
EC_subsubclass_t <- data.frame(EC_subsubclass_t)
EC_subsubclass_t <- rownames_to_column(EC_subsubclass_t, var = "data_to_aggregate")
EC_subsubclass_t_sum <- aggregate()

# another try 
# take a subset of just the ECs
EC_subsubclass_ECs <- EC_subsubclass[,64:2160]
EC_colnames <- colnames()
colnames(EC_subsubclass_ECs) <- colnames()
...
EC_subsubclass_comb <- combine_duplicates(EC_subsubclass)
...
##### code for subclass level plotting when I need it:
plot_bar(p_subclass, x = "SHMI2_bin", fill = "Family") +
  geom_bar(stat = "identity") +
  theme_classic() +
...
EC_split <- str_split(EC_names,"\\.")

EC_split2 <- sapply(EC_split, head, 2)

EC_split3 <- sapply(EC_split2, function(name){})
