...
for (myVar in myVars) {
  ...
  p <- ggplot(all_merged_imp2_unlisted3, aes(x = all_ECs, y = species_freq)) + 
  ...
  assign(paste0("sp_per_ez_",myVar),p)
  ...
  ec_abund <- all_merged_imp2_unlisted_ECcount %>% 
    pivot_wider(names_from = all_ECs, values_from = EC_count_per_species)
  ec_abund <- replace(ec_abund, is.na(ec_abund), 0)
  ec_abund <- column_to_rownames(ec_abund, var = "species")
  ec_abund <- as.matrix(ec_abund)

  # create heatmap
  pal <- colorRampPalette(brewer.pal(9, "Reds"))(25)
  #png(filename = paste0("figures/species_per_enzyme/species_freq_per_ez_heatmap_",myVar,".png"))
  p <- heatmap(ec_abund, main = myVar, col = pal)
  legend(x = "left", cex = 0.8, legend = c("Low", "Medium", "High"), 
         fill = colorRampPalette(brewer.pal(9, "Reds"))(3))
  #dev.off()
  #assign(paste0("heatmap_",myVar),p)
  
  ##### need to play with figure output dimensions. Make wider
  ##### also get fonts to be nicer - smaller taxa names
  
  
}

# create merged plot
p_sp_ez_all <- ggarrange(sp_per_ez_ACE, sp_per_ez_activeC, sp_per_ez_agg_stab,
                         sp_per_ez_fe, sp_per_ez_k, sp_per_ez_mg, sp_per_ez_mn,
                         sp_per_ez_p, sp_per_ez_ph, sp_per_ez_resp, sp_per_ez_SOM,
                         sp_per_ez_water_cap, sp_per_ez_zn, ncol = 4, nrow = 4)
p_sp_ez_all

annotate_figure(p_sp_ez_all, left = text_grob("Species Frequency", rot = 90, vjust = 1, size = 14),
...
for (myVar in myVars) {
 ...
 gDF.neg <- gDF %>% 
  ...
  # write important variables to a csv file
  gDF.pos.grouped <- gDF.pos %>% 
    group_by(Predictor) %>% 
    summarise(mean_VIP = mean(VIP)) %>% 
    mutate(indicator = myVar)
  gDF.neg.grouped <- gDF.neg %>% 
    group_by(Predictor) %>% 
    summarise(mean_VIP = mean(VIP)) %>% 
    mutate(indicator = myVar)
  
  write.table(gDF.pos.grouped, "machine_learning/16S_EC/top_ECs.csv", sep = ",",
              append = TRUE, row.names = FALSE, col.names = FALSE)
  write.table(gDF.neg.grouped, "machine_learning/16S_EC/top_ECs.csv", sep = ",",
              append = TRUE, row.names = FALSE, col.names = FALSE)
