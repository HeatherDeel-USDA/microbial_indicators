...
rangeland_sp <- rangeland_sp[,c(2,4)]
rangeland_sp <- rangeland_sp[!rangeland_sp$species == "",]


############## need to figure out how to join duplicates - there are many of the same species with different EC lists. Can probably just group by species, should perhaps do this with species instead of OTU ID?
##### with some nomenclature formatting, should then be able to join with varimp results
##### but need to be very careful to make sure formatting is consistent with varimp output - get rid of all spaces, perhaps

### merge files, format, and save
all_merged <- hops2018_sp %>% 
  full_join(hopsARS_sp, by = "species", relationship = "many-to-many") %>% 
  full_join(NRCS1_sp, by = "species", relationship = "many-to-many") %>% 
  full_join(NRCS2_sp, by = "species", relationship = "many-to-many") %>% 
  full_join(rangeland_sp, by = "species", relationship = "many-to-many")
