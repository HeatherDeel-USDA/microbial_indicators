...
############ to do:
####### redo important EC and tax lists so it lists just the unique ones - don't need to overwrite files, just edit the object after reimporting
####### redo heatmaps with ggplot2 so I can merge
####### make it so that all 10 enzymes are shown in the heatmaps so we show which ones are redundant and which ones aren't - use full_join instead of left join
####### make color breaks consistent in the heatmaps
####### redo all_merged file using the picrust2 nsti file inputs
...
### graph important ECs in important taxa for each indicator
...
for (myVar in myVars) {
  ...
  all_merged_imp <- all_merged_unique %>% 
    filter(species %in% top_TAX_indic$species) ##### why am I only getting 8 here? shouldn't I be getting 10??? is it that there are some species that aren't in picrust2? e.g., for ace, Dehalogenimonas_formicexedens and Blastococcus_colisei
  ...
  all_merged_imp2_unlisted <- unnest(all_merged_imp2, all_ECs) ### start here with new data import

  # filter by ECs in top ECs list
  all_merged_imp2_unlisted2 <- all_merged_imp2_unlisted %>% 
    filter(all_ECs %in% top_ECs_indic$EC)
