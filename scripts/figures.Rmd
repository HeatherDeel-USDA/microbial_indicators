...
# separating by positively and negatively correlated features (there ended up being no negatively correlated enzymes)
```{r}
gibbs <- read.csv('picrust2_files/GIBBs.EC.numbers.csv')
gibbs <- gibbs %>%
  filter(Missing != 'x')
...
for (myVar in myVars) {
 ...
 ec.mat.pos <- filter(ec.mat, paste0(myVar) > 0)
 ec.mat.neg <- filter(ec.mat, paste0(myVar) < 0)

  # filter out poor indicators in pos and neg correlations
 gDF.pos <- gDF %>% 
   filter(Predictor %in% ec.mat.pos$Predictor | Predictor %in% c('clay','ClimateZ','DNA')) 
 gDF.neg <- gDF %>% 
   filter(Predictor %in% ec.mat.neg$Predictor | Predictor %in% c('clay','ClimateZ','DNA')) 
...
p_gibbs_merged_pos <- ggarrange(AggStab_gibbs_pos, Mg_gibbs_pos, Mn_gibbs_pos, Resp_gibbs_pos,
                            SOM_gibbs_pos, WaterCap_gibbs_pos, Zn_gibbs_pos, p_legend3,
                            nrow = 2, ncol = 4)
p_gibbs_merged_pos
...
ggsave("figures/gibbs_varimps_pos.png", device='png', dpi=600, height=8, width=12)

# negative correlations graph
p_gibbs_merged_neg <- ggarrange(AggStab_gibbs_neg, Mg_gibbs_neg, Mn_gibbs_neg, Resp_gibbs_neg,
                            SOM_gibbs_neg, WaterCap_gibbs_neg, Zn_gibbs_neg, p_legend3,
                            nrow = 2, ncol = 4)
p_gibbs_merged_neg
...
ggsave("figures/gibbs_varimps_neg.png", device='png', dpi=600, height=8, width=12)
```
...
# filter out poor indicators in pos and neg correlations
 # putting this in a loop because P does not have ClimateZ in its model
 if (myVar %in% c('ace', 'activeC', 'k','fe')){
  gDF.pos <- gDF %>% 
    filter(Predictor %in% ec.mat.pos$Predictor | Predictor %in% c('clay','ClimateZ','DNA')) 
  gDF.neg <- gDF %>% 
    filter(Predictor %in% ec.mat.neg$Predictor | Predictor %in% c('clay','ClimateZ','DNA')) 
 ...
 if (myVar %in% c('p', 'ph')){
  gDF.pos <- gDF %>% 
    filter(Predictor %in% ec.mat.pos$Predictor | Predictor %in% c('clay','DNA')) 
  gDF.neg <- gDF %>% 
    filter(Predictor %in% ec.mat.neg$Predictor | Predictor %in% c('clay','DNA')) 
 }
...
# positive correlations graph
p_merged_pos <- ggarrange(ACE_pos, ActiveC_pos, Fe_pos, K_pos, P_pos, pH_pos, 
                          p_legend3,nrow = 2, ncol = 4)
p_merged_pos

annotate_figure(p_merged_pos, left = text_grob("Variable Importance", rot = 90, vjust = 1, size = 14),
                bottom = text_grob("Positively Correlated Model Predictors", size = 14))

ggsave("figures/all_enzyme_varimps_pos.png", device='png', dpi=600, height=8, width=12)

# negative correlations graph
p_merged_neg <- ggarrange(ACE_neg, ActiveC_neg, Fe_neg, K_neg, P_neg, pH_neg, 
                          p_legend3,nrow = 2, ncol = 4)
p_merged_neg
...
ggsave("figures/all_enzyme_varimps_neg.png", device='png', dpi=600, height=8, width=12)
```
