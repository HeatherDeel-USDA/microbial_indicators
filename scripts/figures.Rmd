...
############ to do:
####### redo heatmaps with heatmap.2
####### make it so that all 10 enzymes are shown in the heatmaps so we show which ones are redunant and which ones aren't
####### create better, more accurate legend - or just quantify the range of low, medium, and high for each indicator and put in the figure caption
```{r}
### hops 2018
...
### import varimps and create list of top important taxa
...
### search for important ECs in important taxa - how many times do important ECs appear in the important taxa?
# import important ECs - created in chunk for figures 4 and 5
top_ECs <- read.csv("machine_learning/16S_EC/top_ECs.csv", header = FALSE)
colnames(top_ECs) <- c("Predictor","mean_VIP","indicator")

# import important taxa
top_TAX <- read.csv("machine_learning/16S_TAX/top_TAX.csv", header = FALSE)
colnames(top_TAX) <- c("Predictor","mean_VIP","indicator")

for (myVar in myVars) {
  top_ECs_indic <- top_ECs %>% 
    filter(indicator == myVar)
  top_TAX_indic <- top_TAX %>% 
    filter(indicator == myVar)

  # filter all_merged down to just top taxa for each indicator
  all_merged_imp <- all_merged %>% 
    filter(species %in% top_TAX_indic$Predictor)

  # merge all ECs into one column
  all_merged_imp2 <- all_merged_imp %>%
    mutate(all_ECs = mapply(c, Hops2018_ECs, hopsARS_ECs, NRCS1_ECs,NRCS2_ECs, rangeland_ECs))
  all_merged_imp2 <- all_merged_imp2[,c(1,7)]
  all_merged_imp2_unlisted <- unnest(all_merged_imp2, all_ECs)

  # filter by ECs in top ECs list
  all_merged_imp2_unlisted2 <- all_merged_imp2_unlisted %>% 
    filter(all_ECs %in% top_ECs_indic$Predictor)
  
  # calculate number of species each EC occurs in
  all_merged_imp2_unlisted3 <- all_merged_imp2_unlisted2 %>% 
    group_by(species, all_ECs) %>%
    filter(row_number()==1) %>%
    ungroup() %>%
    group_by(all_ECs) %>% 
  ...
  colnames(all_merged_imp2_unlisted3)[1] <- "Predictor"
  all_merged_imp2_unlisted3 <- all_merged_imp2_unlisted3 %>% 
    left_join(top_ECs_indic, by = "Predictor")
  all_merged_imp2_unlisted3 <- all_merged_imp2_unlisted3[order(all_merged_imp2_unlisted3$mean_VIP, decreasing = TRUE),]
  all_merged_imp2_unlisted3$Predictor <- reorder(all_merged_imp2_unlisted3$Predictor, all_merged_imp2_unlisted3$mean_VIP)
  ...
  p <- ggplot(all_merged_imp2_unlisted3, aes(x = Predictor, y = species_freq)) + 
  ...
  # create abundance df of ECs per species for each indicator
  # start with all_merged_imp2_unlisted
  all_merged_imp2_unlisted_ECcount <- all_merged_imp2_unlisted %>% 
    group_by(all_ECs, species) %>% 
    summarise(EC_count_per_species = n()) %>% 
    filter(all_ECs %in% top_ECs_indic$Predictor)
  
  # join with top ECs indic to reorder ECs by importance
  colnames(all_merged_imp2_unlisted_ECcount)[1] <- "Predictor"
  all_merged_imp2_unlisted_ECcount2 <- all_merged_imp2_unlisted_ECcount %>% 
    left_join(top_ECs_indic, by = "Predictor")
    all_merged_imp2_unlisted_ECcount2 <- all_merged_imp2_unlisted_ECcount2[order(all_merged_imp2_unlisted_ECcount2$mean_VIP),]
  all_merged_imp2_unlisted_ECcount2$Predictor <- reorder(all_merged_imp2_unlisted_ECcount2$Predictor, all_merged_imp2_unlisted_ECcount2$mean_VIP)
  all_merged_imp2_unlisted_ECcount2 <- all_merged_imp2_unlisted_ECcount2[,1:3]
  
  ec_abund <- all_merged_imp2_unlisted_ECcount2 %>% 
    pivot_wider(names_from = Predictor, values_from = EC_count_per_species)
  ec_abund <- replace(ec_abund, is.na(ec_abund), 0)
  
  # join with top_TAX_indic to reorder species by importance
  colnames(ec_abund)[1] <- "Predictor"
  ec_abund <- ec_abund %>% 
    left_join(top_TAX_indic, by = "Predictor") %>% 
    arrange(mean_VIP)
  ec_abund <- column_to_rownames(ec_abund, var = "Predictor")
  ec_abund <- ec_abund %>% 
    select(starts_with("EC:"))
  ec_abund <- as.matrix(ec_abund)

  # create heatmap
  # rows will be arranged from least important taxa on top to most important taxa on bottom. Columns arranged from least to most important EC for that indicator from left to right
  # pal <- colorRampPalette(brewer.pal(9, "Reds"))(22)
  # 
  # # using heatmap.2
  # par(cex.main = 0.8)
  # p_test <- heatmap.2(ec_abund, main = myVar, cexRow = 0.7, cexCol = 0.7, Rowv = FALSE,
  #                     Colv = FALSE, col = pal, dendrogram = "none", trace = "none",
  #                     density.info = "none",breaks=c(0,5,10,15,20,25,50,75,100,150,
  #                                                    250,300,350,400,450,500,750,
  #                                                    1000,1500,2000,2500,3000,3500))
 
 # using ggplot2
  ###### just have to go with this. 
  # Need to refigure out ordering of taxa
  # change colors back to red
  # create consistent color breaks between indicators
  # add in all 10 ECs - use full join instead of left join
 ggplot(all_merged_imp2_unlisted_ECcount2, aes(x = Predictor, y = species, fill = EC_count_per_species)) +
   geom_tile()
