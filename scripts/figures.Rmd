...
####### double check sorting of x and y axes and resort/regen plots if needed
```{r}
...
### Figure X
# PCOAs using tsne
####### still need to output EC and taxonomy statistics to a file (if we are keeping this figure)
```{r}
### EC data PCOA
...
####### Need to redo this with new variable importances in Dan code (now pasted above)
```{r}
myVars <- c('SOM', 'resp', 'agg_stab', 'water_cap', 'mg', 'mn', 'zn', 'ace', 'activeC', 'ph', 'p', 'k','fe')
...
for (myVar in myVars) {
 ...
 write.table(gDF2.grouped, "machine_learning/16S_EC/top_ECs_HLD.csv", sep = ",",
              append = TRUE, row.names = FALSE, col.names = FALSE)
...
### Figure X
# Rooted and unrooted dendrogram
...
hc <- hclust(dd, method = "ward.D2") #cluster analysis on dissimilarity distances
hcd <- as.dendrogram(hc)

# Default plot
plot(hcd, type = "rectangle", ylab = "Height") #rooted

plot(ape::as.phylo(hc), type = "unrooted", cex = 0.6,
     no.margin = TRUE)

### PCA (Dan's)
meta <- data.frame(Level2=df$Level2, Level3=df$Level3)
ord <- rda(data) #ordination on level 3 enzyme data for each indicator

q <- ordiplot(ord, choices=c(1,2), scaling=3, type='none', plot=F) #what are plot and scaling? not in documentation
species <- data.frame(q$species)
species$Level2 <- meta$Level2
wanted <- goeveg::ordiselect(data, ord, fitlim=0.5)
...
p <- ggplot(species) +
  geom_point(data=sites, inherit.aes=F, aes(x=PC1, y=PC2)) +
  geom_text_repel(data=sites, inherit.aes=F, aes(x=PC1, y=PC2, label=row.names(sites))) +
  geom_segment(aes(x=0, y=0, xend=PC1, yend=PC2, color=Level2)) +
  #geom_text_repel(aes(x=PC1, y=PC2, label = species$label, color=Level2), size=2) + # I get an error with this line
  geom_circle(aes(x0=0, y0=0, r=4)) +
  coord_equal() +
  theme_bw()
p

### PCA (Heather's)(it looks awful, but the clustering is really different)
library(ape)
dd2 <- vegdist(data, method = "bray")
res <- pcoa(dd2)
res$values
biplot(res)

data.st = apply(data, 2, scale, center = TRUE, scale = TRUE)
par(mfrow=c(1,2))
biplot(res, data)
biplot(res, data.st)
...
par(mfrow=c(1,2))
biplot(res, data, dir.axis1=-1, dir.axis2=-1)
biplot(res, data.st, dir.axis1=-1, dir.axis2=-1)

# another type of PCA biplot
...
### More of Dan's PCA code (get running here, incorporate into above chunk and delete later)
```{r}
df <- read_xlsx("figures/top_predictors/Summary.xlsx", sheet="All_NZ", range="A2:O57")
...
p <- ggplot(species) +
  ...
  #ggrepel::geom_text_repel(aes(x=Axis1, y=Axis2, label = species$label, color=Level2), size=3, segment.color="grey80") +
  # to get ggrepel to work, try reinstalling phyloseq from github, and updating ggrepel
...
data <- df %>%
  select(-Enzyme) %>%
  as.data.frame()
row.names(data) <- df$Enzyme

data[is.na(data)] <- 0
data <- t(data)
data <- data[,colSums(data) > 0]
#data <- decostand(data, 'range', margin=1)
data <- decostand(data, 'pa')
...
species <- data.frame(q$species)
names(species) <- c('Axis1', 'Axis2')
wanted <- goeveg::ordiselect(data, ord, fitlim=0.25)
species <- species[wanted,]
species$label <- row.names(species)
sites <- data.frame(q$sites)
names(sites) <- c('Axis1', 'Axis2')
...
p <- ggplot(species) +
  ...
  ggrepel::geom_text_repel(data=sites, inherit.aes=F, 
                           aes(x=Axis1, y=Axis2, label=row.names(sites))) +
  geom_segment(aes(x=0, y=0, xend=Axis1, yend=Axis2), color='blue', 
               arrow = arrow(length = unit(0.1, "cm"))) +
  ggrepel::geom_text_repel(aes(x=Axis1, y=Axis2, label = species$label), 
                           size=2, color='blue', max.overlaps=20, segment.color="grey80") +
...
p <- ggplot(sites, aes(x=Axis1, y=Axis2)) +
...
colnames(data_pw3) <- gsub("Biosynthesis_of_other_secondary_metabolites","Biosynth_secondary_metabolites",colnames(data_pw3))
...
plot_list <- list(Amino_acid_metabolism, Biosynthesis_secondary_metabolites,
                  ...
                  Sensory_system, Signal_transduction, Signaling_molecules_and_interaction,
