...
modules <- read.csv("scripts/16S_EC/res_node_table_0.9.csv")
...
for (myVar in myVars) {
  ...
  p
    ggsave(paste0("figures/varimps/", myVar, "_RA_corr_varimp_mod0.9.png"), device='png', dpi=600, height=6, width=8)
...
for (myVar in myVars) {
 ...
 gDF <- varimps_full %>%
  ...
  p <- ggplot(gDF, aes(x=Predictor, y=VIP, fill=module)) +
    ...
    labs(x="", y="Variable Importance")
  p
    ggsave(paste0("figures/varimps/", myVar, "_RA_corr_GIBBs_varimp_mod0.9.png"), device='png', dpi=600, height=6, width=8)
...
# also need to decide which correlation coefficient to use. 0.7?
```
