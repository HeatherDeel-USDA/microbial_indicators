...
# remaking figure resized for FRMS poster
p_heatmap_leg2 <- ggpubr::ggarrange(ace_hm, activeC_hm, agg_stab_hm, fe_hm, k_hm, mg_hm,
                           mn_hm, p_hm, ph_hm, resp_hm, SOM_hm, water_cap_hm, zn_hm,
                           nrow = 4, ncol = 4, common.legend = TRUE)
p_heatmap_leg2

p_heatmap_leg <- annotate_figure(p_heatmap_leg2, left = text_grob("Important Taxa", rot = 90, vjust = 1, size = 14),
                bottom= text_grob("Important Enzymes", size = 14))

ggsave(plot=p_heatmap_leg2, "figures/heatmap_dkm/ez_copies_per_species_heatmap2_FRMS.png", 
       device='png', dpi=600, height=18, width=20)
...
rownames(data) <- c("SOM","ActiveC","ACE","Resp","AggStab","WaterCap","pH",
                     "P","K","Fe","Mg","Mn","Zn")
...
plot(ape::as.phylo(hc), type = "unrooted", cex = 1, no.margin = TRUE)
# manually saved as png to "C:\Users\Heather.Deel\OneDrive - USDA\microbial_indicators\microbial_indicators\microbial_indicators\figures\unrooted_dendrogram.png"
...
ggsave("figures/biplot.png", device='png', dpi=600, height=4, width=8)

# manually combined dendrogram and biplot in dendro_pathways.pptx due to low quality of dendrogram when merging with p2

```

### Figure X
# PCA of SHMI 
####### Leaving code here for now, but not including it in the manuscript for now
```{r}
...
p4 <- ggplot(sites, aes(x=Axis1, y=Axis2)) +
  ...
  geom_point(aes(color=meta$SHMI2_bin, shape=meta$Current_Land_Use)) +
  #facet_wrap(~ meta$ClimateZ) +
  #scale_shape_manual(values=c(21,22,23,24), labels=c("Annual Cropland","Perennial Cropland","Rangeland")) +
  #guides(fill=guide_legend(override.aes = list(size=4, shape=21))) +  
  #geom_text_repel(aes(label=row.names(sites))) +
  coord_equal() +
  theme_bw()
p4
```
...
p_all <- ggpubr::ggarrange(plotlist = plot_list, nrow = 6, ncol = 6)
p_all
...
p_sig <- ggpubr::ggarrange(plotlist = plot_list2, nrow = 6, ncol = 4)
p_sig
...
# subset to non-human categories and just those I want to show on my FRMS 2024 poster
plot_list3 <- list(Amino_acid_metabolism, Biosynthesis_secondary_metabolites,
                  Carbohydrate_metabolism, Cellular_community_prokaryotes,
                  Chromosome, Energy_metabolism, Folding_sorting_degradation, 
                  Glyc_biosynthesis_metabolism, Lipid_metabolism,
                  Membrane_transport, Metabolism_cofactors_vitamins,
                  Metabolism_other_amino_acids, Metabolism_terpenoids_polyketides,
                  Nucleotide_metabolism, Replication_and_repair,Transcription, 
                  Transport_and_catabolism,Xeno_biodegradation_metabolism)

p_sig <- ggpubr::ggarrange(plotlist = plot_list3, nrow = 4, ncol = 5)
p_sig

annotate_figure(p_sig, left = text_grob("Relative Abundance", rot = 90, vjust = 1, size = 14),
                bottom = text_grob("SHMI Bin", size = 14))
 
ggsave("figures/metabolisms_SHMI/SHMI_sig_FRMS.png", device='png', dpi=600, height=9, width=11)
```
