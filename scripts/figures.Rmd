...
# to do:
# figure out how to create and merge pdps within the loop
# decide if we want to include clay and climate in these - climate is weird because it's categorical
```{r}
...
for (myVar in MyVars) {
  filenames <- list.files(paste0("machine_learning/16S_FINAL/",myVar,"_model_results/"), pattern = "ace_final_pd_Predictor_EC*", full.names = TRUE)
  ...
  graph <- lapply(predictors, function(x)
    p <- ggplot(x, aes(x = predictor_value, y = indicator_response, group = factor(predictor))) +
    geom_line() +
