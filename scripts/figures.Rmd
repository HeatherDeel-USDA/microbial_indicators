...
# try merging the lists together
### this seems to work, can add all dfs and merge into one column, then try to use the unique function to pipe the lists into another column and potentially separate that out into their own cells if needed
all_merged <- all_merged %>% 
  mutate(all_ECs = mapply(c, Hops2018_ECs, HopsARS_ECs, NRCS1_ECs,
                          NRCS2_ECs, Rangeland_ECs))
all_merged_2 <- all_merged[,c(1,7)]
all_merged_2$unique_ECs <- unique(all_merged_2$all_ECs)
