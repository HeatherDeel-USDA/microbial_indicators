...
```{r}
# ref code - figure out how to assign ECs to each module and then determine whether there's a GIBBs enzyme in that module
...
for (myVar in myVars) {
  ...
  p <- ggplot(gDF, aes(x=Predictor, y=VIP)) +
    ...
    geom_boxplot(alpha = 1, fatten = 1) + 
    theme_bw() +
    theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5), plot.title = element_text(size = 12)) + # will need to add legend.position = "none" to this line eventually
    labs(x="", y="", title = myVar)
  p
