...
# get row names back to a column

EC_sam_SHMI_SEM <- EC_sam[,c()]
...
ECs_mod1 <- ECs_mod %>% 
  filter(module == 1)
ECs_mod1 <- ECs_mod1[,1:536] # get rid of module col from otu table
OTU1 = otu_table(ECs_mod1, taxa_are_rows = TRUE)
p_mod1 <- phyloseq(OTU1, TAX, SAM)
p_mod1_melt <- psmelt(p_mod1)
p_mod1_persamp <- p_mod1_melt %>% 
  group_by(Sample) %>% 
  summarize(Abundance = sum(Abundance))
p_mod1_persamp$module <- "1"
...
p_mod126_melt$module <- "126"
################ after reduced sample file created, merge with mod files here

# merge module 1 and module 126 dfs
ace_EC_mods <- full_join(p_mod1_melt, p_mod126_melt)

# line graph
p_mod_ace <- ggplot(ace_EC_mods, aes(x = SHMI2_rating, y = Abundance, color = module)) +
  geom_line()
p_mod_ace

# need to group by sample in psmelt objects


# ACE
# filter down to just one ace module
# then psmelt
# then add column with module name
# do for each module, then merge all modules I want to graph at the end
# then do a single line graph with lines colored by module
# go back and simplify code/comments so it's easier to do this 1000 times
