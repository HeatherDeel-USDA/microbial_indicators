...
############ to do:
# recreate number of species per enzyme bar charts after sorting enzymes from most to least importance
# merge figures (bar charts and heatmaps) to final figures
```{r}
### hops 2018
...
for (myVar in myVars) {
  ...
  pal <- colorRampPalette(brewer.pal(9, "Reds"))(25)
  
  png(filename = paste0("figures/species_per_enzyme/species_freq_per_ez_heatmap_",myVar,".png"), width = 6.5, height = 3, units = "in", res = 1200)
  par(cex.main = 0.8)
  p2 <- heatmap(ec_abund, main = myVar, col = pal, cexRow = 0.7, cexCol = 0.7, Rowv = NA, Colv = NA)

  legend(x = "left", cex = 0.8, legend = c("Low", "Medium", "High"), fill = colorRampPalette(brewer.pal(9, "Reds"))(3))
  dev.off()
}
...
# merged plot of heatmaps created in powerpoint due to difficult format output by the heatmap function
