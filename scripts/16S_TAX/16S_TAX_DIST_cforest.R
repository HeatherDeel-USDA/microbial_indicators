...
ml_TAX_16S_DIST <- ml_TAX_16S[,c(70,104:110,101,152:7614)]

# filter NAs
ml_TAX_16S_DIST$clay <- as.numeric(ml_TAX_16S_DIST$clay)
ml_TAX_16S_DIST$dist <- as.numeric(ml_TAX_16S_DIST$dist)
ml_TAX_16S_DIST <- ml_TAX_16S_DIST %>%
  filter(!is.na(dist))

# format so : and . are repldistd by _ (for varimp)
names(ml_TAX_16S_DIST) <- gsub(":","_", names(ml_TAX_16S_DIST))
names(ml_TAX_16S_DIST) <- gsub("\\.","_", names(ml_TAX_16S_DIST))

# split into train and test (4/5 proportion)
ml_TAX_16S_DIST$id <- 1:nrow(ml_TAX_16S_DIST)
train <- ml_TAX_16S_DIST %>% dplyr::sample_frac(0.80)
test <- dplyr::anti_join(ml_TAX_16S_DIST, train, by = 'id')
...
saveRDS(cf.dist, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/DIST_model_results_clay_climate/cf.dist", args[1], ".RDS", sep = ""))
...
test.dist <- data.frame(test[,9])
colnames(test.dist)[1] <- "dist.obs"
test.dist <- rownames_to_column(test.dist, var = "id")
cf.pvso <- merge(cf.pred, test.dist, by = "id")

saveRDS(cf.pvso, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/DIST_model_results_clay_climate/cf_obs_vs_pred", args[1], ".RDS", sep = ""))
...
ggsave(paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/DIST_model_results_clay_climate/DIST_TAX_pred_vs_obs", args[1], ".pdf", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "pdf")
...
write.table(cbind(task_num,r2_val,p_val), 
            file = "/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/DIST_model_results_clay_climate/dist.ec.stats.csv", 
...
write.csv(dist.imp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/DIST_model_results_clay_climate/DIST_TAX_var_importance", args[1], ".csv", sep = ""), row.names = TRUE)
