...
ml_TAX_16S_ROOT <- ml_TAX_16S[,c(70,104:110,99,152:7614)]

# filter NAs
ml_TAX_16S_ROOT$clay <- as.numeric(ml_TAX_16S_ROOT$clay)
ml_TAX_16S_ROOT$root <- as.numeric(ml_TAX_16S_ROOT$root)
ml_TAX_16S_ROOT <- ml_TAX_16S_ROOT %>%
  filter(!is.na(root))

# format so : and . are replrootd by _ (for varimp)
names(ml_TAX_16S_ROOT) <- gsub(":","_", names(ml_TAX_16S_ROOT))
names(ml_TAX_16S_ROOT) <- gsub("\\.","_", names(ml_TAX_16S_ROOT))

# split into train and test (4/5 proportion)
ml_TAX_16S_ROOT$id <- 1:nrow(ml_TAX_16S_ROOT)
train <- ml_TAX_16S_ROOT %>% dplyr::sample_frac(0.80)
test <- dplyr::anti_join(ml_TAX_16S_ROOT, train, by = 'id')
...
saveRDS(cf.root, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/ROOT_model_results_clay_climate/cf.root", args[1], ".RDS", sep = ""))
...
test.root <- data.frame(test[,9])
colnames(test.root)[1] <- "root.obs"
test.root <- rownames_to_column(test.root, var = "id")
cf.pvso <- merge(cf.pred, test.root, by = "id")

saveRDS(cf.pvso, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/ROOT_model_results_clay_climate/cf_obs_vs_pred", args[1], ".RDS", sep = ""))
...
ggsave(paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/ROOT_model_results_clay_climate/ROOT_TAX_pred_vs_obs", args[1], ".pdf", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "pdf")
...
write.table(cbind(task_num,r2_val,p_val), 
            file = "/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/ROOT_model_results_clay_climate/root.ec.stats.csv", 
...
write.csv(root.imp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/ROOT_model_results_clay_climate/ROOT_TAX_var_importance", args[1], ".csv", sep = ""), row.names = TRUE)
