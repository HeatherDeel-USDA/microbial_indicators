...
ml_TAX_16S_SEMWISE <- ml_TAX_16S[,c(70,104:110,112,152:7614)]

# filter NAs
ml_TAX_16S_SEMWISE$clay <- as.numeric(ml_TAX_16S_SEMWISE$clay)
ml_TAX_16S_SEMWISE$SH_rating <- as.numeric(ml_TAX_16S_SEMWISE$SH_rating)
ml_TAX_16S_SEMWISE <- ml_TAX_16S_SEMWISE %>%
  filter(!is.na(SH_rating))

# format so : and . are replSH_ratingd by _ (for varimp)
names(ml_TAX_16S_SEMWISE) <- gsub(":","_", names(ml_TAX_16S_SEMWISE))
names(ml_TAX_16S_SEMWISE) <- gsub("\\.","_", names(ml_TAX_16S_SEMWISE))

# split into train and test (4/5 proportion)
ml_TAX_16S_SEMWISE$id <- 1:nrow(ml_TAX_16S_SEMWISE)
train <- ml_TAX_16S_SEMWISE %>% dplyr::sample_frac(0.80)
test <- dplyr::anti_join(ml_TAX_16S_SEMWISE, train, by = 'id')
...
saveRDS(cf.SH_rating, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/SEMWISE_model_results_clay_climate/cf.SH_rating", args[1], ".RDS", sep = ""))
...
test.SH_rating <- data.frame(test[,9])
...
saveRDS(cf.pvso, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/SEMWISE_model_results_clay_climate/cf_obs_vs_pred", args[1], ".RDS", sep = ""))
...
ggsave(paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/SEMWISE_model_results_clay_climate/SEMWISE_TAX_pred_vs_obs", args[1], ".pdf", sep = ""), unit = "in", width = 6, height = 6, dpi = 300, device = "pdf")
...
write.table(cbind(task_num,r2_val,p_val), 
            file = "/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/SEMWISE_model_results_clay_climate/SH_rating.ec.stats.csv", 
...
write.csv(SH_rating.imp, paste("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/SEMWISE_model_results_clay_climate/SEMWISE_TAX_var_importance", args[1], ".csv", sep = ""), row.names = TRUE)
