...
# note that this is not copy number corrected ("corr") - want to leave picrust2 out of this

for (myVar in myVars) {
  data <- ml_TAX_16S[,152:7614]
  df.myVar <- as.vector(ml_TAX_16S[,myVar])
  df.climate <- as.vector(ml_TAX_16S[,'ClimateZ'])
  df.clay <- as.vector(ml_TAX_16S[,'clay'])
  df.dna <- as.vector(ml_TAX_16S[,'DNA'])
  ...
  # format so . are replaced by _ (for varimp)
  ...
  write.table(cbind(args[1], r2_val, p_val, myVar, more_stats),
              file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/", myVar, "_model_results_RA_TAX/", myVar, "_RA_TAX_stats", ".csv", sep = ""),
  ...
  imp <- tibble::rownames_to_column(imp, "TAX")
  imp$Run <- args[1]
  
  # write variable importances to a file
  # writing each to their own file because scinet was messing up 
  write.table(imp,
              file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/", myVar, "_model_results_RA_TAX/", myVar, "_RA_TAX_varimp", args[1],".csv", sep = ""),
              col.names = FALSE, append = TRUE, sep = ",", row.names = FALSE)
  for (TAX in seq(1,min(10,length(imp$TAX)),1)) {
    print(paste0("Partial dependence for predictor ", TAX, ": ", imp$TAX[TAX]))
    
    pd <- GetPartialData(cf.train, xnames=imp$TAX[TAX], 
                         quantiles=FALSE, grid.resolution = NULL,
                         parallel=TRUE)
    pd$run <- args[1]
    
    write.table(pd, 
                file = paste0("/project/soil_micro_lab/micro_indicators/machine_learning/16S_TAX/", myVar, "_model_results_RA_TAX/", myVar, "_RA_TAX_pd", "_Predictor_", imp$TAX[TAX], ".csv", sep = ""),
                col.names=FALSE,
